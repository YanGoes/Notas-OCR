...
function periodoDoTexto(texto) {
    ...
    for (const [periodo, apelidos] of APELIDOS) {
        if (apelidos.some((apelido) => (` ${alvo} `).includes(` ${normalizar(apelido)} `))) return periodo;
    }
    return null;
}
...
function resolverRefeicao({ hora, horasAlternativas = [], horaOrigem, legenda, pessoas, faixas = [] } = {}) {
    ...
    const horasCandidatas = [...new Set([hora, ...horasAlternativas].map(horaNormalizada).filter(Boolean))];
    const periodosCandidatos = [...new Set(horasCandidatas.map((candidata) => periodoDaHora(candidata, faixas)?.periodo).filter(Boolean))];
    return {
        periodo,
        ...
        hora_origem: porHora ? (horaOrigem || "hora_do_comprovante") : null,
        hora: horaNormalizada(hora),
        horas_candidatas: horasCandidatas,
        periodos_hora_candidatos: periodosCandidatos,
        hora_ambigua: periodosCandidatos.length > 1,
