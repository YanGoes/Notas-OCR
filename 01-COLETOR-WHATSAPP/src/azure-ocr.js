...
// O modelo de recibos nem sempre estrutura TransactionTime. Procura primeiro horas
// rotuladas ou imediatamente depois de uma data. Também preserva todas as horas fortes:
// se duas delas caírem em períodos de refeição diferentes, a validação pede conferência.
function detalhesHoraDoTexto(texto) {
    const conteudo = String(texto || "");
    const fortes = [
        { origem: "hora_rotulada", regex: /\bhora(?:\s*d[ae]\s*\w+)?\s*:?\s*([01]?\d|2[0-3])\s*[:h]\s*([0-5]\d)\b/gi },
        { origem: "data_e_hora", regex: /\b\d{1,2}[\/-](?:\d{1,2}|[A-Z]{3})[\/-]\d{2,4}\s+(?:[àa]s?\s+)?([01]?\d|2[0-3])\s*[:h]\s*([0-5]\d)\b/gi },
        { origem: "evento_rotulado", regex: /\b(?:emiss[aã]o|autoriza[cç][aã]o|venda|compra)\D{0,35}([01]?\d|2[0-3])\s*[:h]\s*([0-5]\d)\b/gi },
    ];
    const candidatos = [];
    const vistos = new Set();
    const adicionar = (achado, origem) => {
        const hora = `${achado[1].padStart(2, "0")}:${achado[2]}`;
        if (vistos.has(hora)) return;
        vistos.add(hora);
        candidatos.push({ hora, origem, indice: achado.index });
    };
    for (const { origem, regex } of fortes) {
        for (const achado of conteudo.matchAll(regex)) adicionar(achado, origem);
    }
    if (!candidatos.length) {
        const solto = /\b([01]?\d|2[0-3])\s*[:h]\s*([0-5]\d)(?::[0-5]\d)?\b(?!\s*:\s*\d)/g;
        const achado = solto.exec(conteudo);
        if (achado) adicionar(achado, "hora_sem_rotulo");
    }
    return {
        hora: candidatos[0]?.hora || null,
        candidatos: candidatos.map((item) => item.hora),
        detalhes: candidatos,
    };
}

function horaDoTexto(texto) {
    return detalhesHoraDoTexto(texto).hora;
}
...
function normalizarResultadoAzure(bruto, modelId = "prebuilt-receipt") {
    ...
    const detalhesHora = horaEstruturada
        ? { hora: horaEstruturada, candidatos: [horaEstruturada] }
        : detalhesHoraDoTexto(texto);
    const horaTexto = horaEstruturada ? null : detalhesHora.hora;
    ...
    return {
        ...
        hora_candidatos: detalhesHora.candidatos,
        valor: valorFinal,
        itens,
...
module.exports = {
    ...
    horaDoTexto, detalhesHoraDoTexto, placaDoTexto, kmDoTexto, enderecoDoCampo, itensDosCampos, tipoSugeridoAzure,
