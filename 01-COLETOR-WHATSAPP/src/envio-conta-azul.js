...
} = require("./fluxo-direto-conta-azul");
...
const ESTADOS_PILOTO_ATIVO = new Set([
    ...
    "DIRETO_AGENDADO", "DIRETO_ENVIANDO", "DIRETO_AGUARDANDO_CONFIRMACAO", "DIRETO_INCERTO",
]);
...
async function verificarDocumento(base, idEmpresa) {
    ...
    try {
        ...
        const empresaId = String(empresa.id);
        let { auditoria } = carregarAuditoria(nomeBase);
        if (auditoria.conta_azul?.modo_lancamento === "DIRETO_ASSISTIDO" || ESTADOS_DIRETO.has(auditoria.conta_azul?.status)) {
            throw new Error("Esta nota seguiu pelo caminho direto assistido. Use a verificacao por protocolo; a previa da Captura nao pode mais ser aceita pelo programa.");
        }
...
async function executarLancamentoDiretoAgendado(base, idEmpresa, agendamentoId) {
    const nomeBase = baseSegura(base);
    if (emExecucao.has(nomeBase)) throw new Error("Este documento ja esta sendo processado.");
    emExecucao.add(nomeBase);
    try {
        const { auditoria } = carregarAuditoria(nomeBase);
        const conta = auditoria.conta_azul || {};
        if (conta.status !== "DIRETO_AGENDADO" || !agendamentoCompativel(conta, "DIRETO", idEmpresa, agendamentoId)) {
            throw new Error("O agendamento persistido do lancamento direto nao corresponde a este trabalho.");
        }
        const direto = conta.direto || {};
        atualizarAuditoria(nomeBase, (anterior) => ({
            ...anterior,
            status: "DIRETO_ENVIANDO",
            modo_lancamento: "DIRETO_ASSISTIDO",
            execucao_instancia: INSTANCIA_PROCESSO,
            erro: null,
            direto: {
                ...(anterior.direto || {}),
                status: "DIRETO_ENVIANDO",
                post_iniciado_em: new Date().toISOString(),
                pendencias: PENDENCIAS_DIRETO,
            },
            tentativas: registrarTentativa(anterior, "DIRETO_POST", "INICIADO"),
        }));

        const resultado = await executarCriacaoDireta(direto.esperado, { empresaId: String(idEmpresa) });
        if (resultado.resposta) salvarResposta(nomeBase, "direto_post", resultado.resposta);
        const agora = new Date().toISOString();
        return atualizarAuditoria(nomeBase, (anterior) => ({
            ...anterior,
            status: resultado.status,
            modo_lancamento: "DIRETO_ASSISTIDO",
            protocolo_direto: resultado.protocolo || anterior.protocolo_direto || null,
            agendamento: null,
            execucao_instancia: null,
            token_confirmacao: null,
            ...
            direto: {
                ...(anterior.direto || {}),
                status: resultado.status,
                protocolo: resultado.protocolo || anterior.direto?.protocolo || null,
                status_remoto: resultado.resposta?.status || null,
                resposta_recebida_em: agora,
                motivo: resultado.motivo || null,
                pendencias: PENDENCIAS_DIRETO,
                reenvio_permitido: false,
            },
            tentativas: registrarTentativa(
                anterior,
                "DIRETO_POST",
                resultado.status === "DIRETO_AGUARDANDO_CONFIRMACAO" ? "PROTOCOLO_RECEBIDO" : "INCERTO_SEM_REENVIO",
                resultado.erro,
            ),
        })).conta_azul;
    } finally {
...
async function agendarLancamentoDireto(base, { idEmpresa, token, idContaFinanceira } = {}) {
    const nomeBase = baseSegura(base);
    if (emExecucao.has(nomeBase)) throw new Error("Este documento ja esta sendo processado.");
    emExecucao.add(nomeBase);
    let trabalho = null;
    try {
        ...
        const prontidao = avaliarProntidao(auditoria, {
            imagemExiste: Boolean(imagem),
            confiancaMinima: Number(regrasAtuais().confianca_minima || 0.95),
            statusPermitidos: ["PREVIA_DIVERGENTE"],
        });
        const elegibilidade = avaliarElegibilidadeDireta(auditoria, { empresaId, imagemExiste: Boolean(imagem) });
        const impedimentos = [...new Set([...prontidao.impedimentos, ...elegibilidade.impedimentos])];
        if (impedimentos.length) throw new Error(impedimentos.join(" "));
        const tokenAtual = tokenDireto(nomeBase, auditoria, empresaId);
        if (!token || token !== tokenAtual) throw new Error("Os dados da nota ou da empresa mudaram. Atualize o painel e confirme novamente.");
        await validarCatalogosRemotos(auditoria);
        const contaFinanceiraId = String(idContaFinanceira || "").trim();
        if (!uuidValido(contaFinanceiraId)) throw new Error("Escolha uma conta financeira valida para o lancamento direto assistido.");
        const contasFinanceiras = await listarContasFinanceiras();
        if (!contasFinanceiras.some((item) => String(item.id || "") === contaFinanceiraId && item.ativo !== false)) {
            throw new Error("A conta financeira escolhida nao existe ou nao esta ativa na empresa conectada.");
        }
        const esperadoDireto = {
            ...elegibilidade.esperado,
            idContaFinanceira: contaFinanceiraId,
            observacao: "Lancamento direto assistido. A imagem permanece na Conta AI Captura e deve ser anexada manualmente.",
            nota: "Conferir fornecedor, comprovante, forma de pagamento e baixa no Conta Azul.",
        };

        const agendamento = novoAgendamento("DIRETO", empresaId);
        atualizarAuditoria(nomeBase, (anterior) => ({
            ...anterior,
            status: "DIRETO_AGENDADO",
            empresa_id: empresaId,
            modo_lancamento: "DIRETO_ASSISTIDO",
            agendamento,
            execucao_instancia: null,
            ...
            direto: {
                status: "DIRETO_AGENDADO",
                empresa_id: empresaId,
                esperado: esperadoDireto,
                snapshot: snapshotConfirmacao(nomeBase, auditoria),
                token_confirmado: tokenAtual,
                pendencias: PENDENCIAS_DIRETO,
                criado_em: new Date().toISOString(),
                reenvio_permitido: false,
            },
            tentativas: registrarTentativa(anterior, "AGENDAR_DIRETO", "AGENDADO"),
        }));
        trabalho = { base: nomeBase, empresaId, agendamentoId: agendamento.id };
    } finally {
        emExecucao.delete(nomeBase);
    }

    filaTrabalho = filaTrabalho.then(async () => {
        try { await executarLancamentoDiretoAgendado(trabalho.base, trabalho.empresaId, trabalho.agendamentoId); }
        catch (erro) {
            console.error(`Conta Azul - direto ${trabalho.base}: ${erro.message}`);
            try {
                const atual = carregarAuditoria(trabalho.base).auditoria.conta_azul || {};
                if (["DIRETO_AGENDADO", "DIRETO_ENVIANDO"].includes(atual.status)) {
                    atualizarAuditoria(trabalho.base, (anterior) => ({
                        ...anterior,
                        ...
                        execucao_instancia: null,
                        token_confirmacao: null,
                        erro: "O caminho direto foi interrompido. Nao reenvie; confira o Conta Azul.",
                        direto: {
                            ...(anterior.direto || {}),
                            status: "DIRETO_INCERTO",
                            reenvio_permitido: false,
                        ...
                        tentativas: registrarTentativa(anterior, "DIRETO_POST", "INCERTO_SEM_REENVIO", erro.message),
                    }));
                }
            } catch (_) { /* o estado persistido continua sendo a fonte de verdade */ }
        }
    });
    return { agendado: true, id_agendamento: trabalho.agendamentoId };
}

async function verificarLancamentoDireto(base, idEmpresa) {
    const nomeBase = baseSegura(base);
    if (emExecucao.has(nomeBase)) throw new Error("Este documento ja esta sendo processado.");
    emExecucao.add(nomeBase);
    try {
        ...
        const { auditoria } = carregarAuditoria(nomeBase);
        const conta = auditoria.conta_azul || {};
        if (conta.modo_lancamento !== "DIRETO_ASSISTIDO" || !ESTADOS_DIRETO.has(conta.status)) {
            throw new Error("Esta nota nao iniciou o caminho direto assistido.");
        }
        if (String(conta.empresa_id || "") !== empresaId || String(conta.direto?.empresa_id || "") !== empresaId) {
            throw new Error("O lancamento direto pertence a outra empresa do Conta Azul.");
        }
        if (conta.status === "DIRETO_CONFIRMADO_PENDENCIAS") return conta;
        if (["DIRETO_AGENDADO", "DIRETO_ENVIANDO"].includes(conta.status)) throw new Error("O POST direto ainda esta em andamento. Nao clique novamente.");
        if (!String(conta.direto?.protocolo || "").trim()) {
            throw new Error("O POST nao devolveu protocolo. Nao e seguro reenviar nem confirmar automaticamente; confira manualmente no Conta Azul.");
        }
        await validarCatalogosRemotos(auditoria);
        const resultado = await executarVerificacaoDireta(conta.direto);
        if (resultado.resultado) salvarResposta(nomeBase, "direto_verificacao", resultado.resultado);
        const eventoId = resultado.resultado?.detalhe?.evento?.id
            || resultado.resultado?.lancamento?.id_evento
            || resultado.resultado?.lancamento?.evento_id
            || null;
        const agora = new Date().toISOString();
        return atualizarAuditoria(nomeBase, (anterior) => ({
            ...anterior,
            status: resultado.status,
            modo_lancamento: "DIRETO_ASSISTIDO",
            evento_id_direto: resultado.confirmado ? eventoId : anterior.evento_id_direto || null,
            erro: resultado.erro || null,
            agendamento: null,
            execucao_instancia: null,
            token_confirmacao: null,
            direto: {
                ...(anterior.direto || {}),
                status: resultado.status,
                confirmado_em: resultado.confirmado ? agora : anterior.direto?.confirmado_em || null,
                evento_id: resultado.confirmado ? eventoId : anterior.direto?.evento_id || null,
                ultima_verificacao_em: agora,
                verificacoes: Number(anterior.direto?.verificacoes || 0) + 1,
                pendencias: PENDENCIAS_DIRETO,
                reenvio_permitido: false,
            },
            tentativas: registrarTentativa(
                anterior,
                "VERIFICAR_DIRETO",
                resultado.confirmado ? "CONFIRMADO_COM_PENDENCIAS" : resultado.status,
                resultado.erro,
            ),
        })).conta_azul;
    } finally {
...
function recuperarEstadoInterrompido(base, auditoria) {
    ...
    const agendamentoOutraInstancia = ["PREPARACAO_AGENDADA", "CONFIRMACAO_AGENDADA", "DIRETO_AGENDADO"].includes(status)
        && String(conta.agendamento?.instancia_id || "") !== INSTANCIA_PROCESSO;
    const execucaoOutraInstancia = ["PREPARANDO", "CONFIRMANDO", "DIRETO_ENVIANDO"].includes(status)
    ...
    } else if (["DIRETO_AGENDADO", "DIRETO_ENVIANDO"].includes(status)) {
        novoStatus = "DIRETO_INCERTO";
        mensagem = "O processo reiniciou durante o caminho direto. O POST pode ter ocorrido; nunca reenvie automaticamente e confira o Conta Azul.";
    } else {
        return auditoria;
    }
    ...
    return atualizarAuditoria(base, (anterior) => ({
        ...
        direto: novoStatus === "DIRETO_INCERTO" ? {
            ...(anterior.direto || {}),
            status: "DIRETO_INCERTO",
            reenvio_permitido: false,
            pendencias: PENDENCIAS_DIRETO,
...
function itemFila(base, auditoria) {
    ...
    const empresaDireta = String(auditoria?.conta_azul?.empresa_id || "");
    const prontidaoDireta = avaliarProntidao(auditoria, {
        imagemExiste: Boolean(imagem && imagem.pasta === "simulacao"),
        confiancaMinima: Number(regrasAtuais().confianca_minima || 0.95),
        statusPermitidos: ["PREVIA_DIVERGENTE"],
    });
    const elegibilidadeDireta = avaliarElegibilidadeDireta(auditoria, {
        empresaId: empresaDireta,
        imagemExiste: Boolean(imagem && imagem.pasta === "simulacao"),
    });
    const diretoDisponivel = prontidaoDireta.pronto && elegibilidadeDireta.permitido;
    return {
        base,
        ...
        direto_disponivel: diretoDisponivel,
        token_direto: diretoDisponivel ? tokenDireto(base, auditoria, empresaDireta) : null,
        impedimentos_direto: [...new Set([...prontidaoDireta.impedimentos, ...elegibilidadeDireta.impedimentos])],
...
module.exports = {
    ...
    agendarLancamentoDireto, verificarLancamentoDireto,
