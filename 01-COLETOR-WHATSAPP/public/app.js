...
function classeContaAzul(status) {
  ...
  if (["PREVIA_DIVERGENTE", "PREPARACAO_INCERTA", "CONFIRMACAO_INCERTA"].includes(status)) return "ca-warning";
  if (["ERRO_PREPARACAO", "REJEITADO"].includes(status)) return "ca-error";
  if (["PREPARACAO_AGENDADA", "PREPARANDO", "CONFIRMACAO_AGENDADA", "CONFIRMANDO"].includes(status)) return "ca-working";
  return "ca-ready";
}
