...
const { resolverRefeicao, periodoDaHora, periodoDoTexto, descricaoRefeicao } = require("../src/refeicao");
const { horaDoTexto, detalhesHoraDoTexto } = require("../src/azure-ocr");
...
test("horario do comprovante prevalece sobre a legenda", () => {
    const resultado = resolverRefeicao({ hora: "12:30", horaOrigem: "heuristica_texto_azure", legenda: "Jantar", pessoas: 2, faixas });
...
test("periodo escrito exige palavra inteira", () => {
    assert.equal(periodoDoTexto("Tipo: café da manhã"), "CAFÉ DA MANHÃ");
    assert.equal(periodoDoTexto("Observacao: comprar cafeina"), null);
    assert.equal(periodoDoTexto("Fornecedor Cafeteria Central"), null);
});
...
test("nao inventa hora a partir de numeros do cupom", () => {
    for (const texto of ["CNPJ 12.345.678/0001-90", "TOTAL R$ 123,45", "COO 123456", "SERIE 12:345", "EC 11:54:74", ""]) {
        assert.equal(horaDoTexto(texto), null, `inventou hora em "${texto}"`);
    }
});

test("preserva horarios fortes distintos para detectar ambiguidade", () => {
    const detalhes = detalhesHoraDoTexto([
        "EMISSAO 08/08/2026 10:29",
        "AUTORIZACAO 08/08/2026 10:31",
    ].join("\n"));
    assert.equal(detalhes.hora, "10:29");
    assert.deepEqual(detalhes.candidatos, ["10:29", "10:31"]);

    const refeicao = resolverRefeicao({
        hora: detalhes.hora,
        horasAlternativas: detalhes.candidatos,
        pessoas: 2,
        faixas,
    });
    assert.equal(refeicao.hora_ambigua, true);
    assert.deepEqual(refeicao.periodos_hora_candidatos, ["CAFÉ DA MANHÃ", "ALMOÇO"]);
});

test("horarios repetidos ou do mesmo periodo nao criam falsa divergencia", () => {
    const refeicao = resolverRefeicao({
        hora: "18:01",
        horasAlternativas: ["18:01", "18:06"],
        pessoas: 2,
        faixas,
    });
    assert.equal(refeicao.hora_ambigua, false);
    assert.deepEqual(refeicao.periodos_hora_candidatos, ["JANTAR"]);
});
...
test("horarios em periodos diferentes vao para revisao", () => {
    const operador = interpretarLegenda("Tipo: Refeicao\nCentro de custo: CONSOL MG-050\nPessoas: 2");
    const ocr = { ...ocrOk, hora: "10:29", hora_candidatos: ["10:29", "10:31"] };
    const classificacao = classificar(operador, ocr);
    const resultado = validar({ operador, classificacao, regras, duplicado: false, ocr });
    assert.equal(classificacao.refeicao.hora_ambigua, true);
    assert.match(resultado.motivos.join(" "), /horarios que caem em periodos diferentes/);
});
